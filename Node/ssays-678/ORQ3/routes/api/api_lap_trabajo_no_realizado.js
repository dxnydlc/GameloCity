...
// api_lap_trabajo_no_realizado.js
...
var _NombreDoc = 'api_lap_trabajo_no_realizado';
...
const { LapTrabajoNoRealizadoModel, LapTrabajoNoRealizadoDetModel, User, sequelize, archiGoogleModel, sucursalModel } = require('../../db');
...
async function execQuery( _where , _limit  )
{
    ...
    //
    _dataResp = await LapTrabajoNoRealizadoModel.findAll({
...
router.post('/', _Requeridos ,async (req,res)=>{
    ...
    try {
        ...
        if( _dataGuardado )
        {
            ...
            _Codigo = 'TNR'+_Codigo;
            await LapTrabajoNoRealizadoModel.update({
                Codigo : _Codigo
            },{
                where  : {
                    uu_id : req.body.uu_id
            ...
            // Unir con detalle.
            await LapTrabajoNoRealizadoDetModel.update({
                CodigoHead : _Codigo
            },{
                where : {
            ...
            var _file = await convertirPDF( 'XLS_TRABAJO_NO_REALIZADO_LAP' , _Codigo );
            _response.resp = { 'titulo' : `Correcto` , 'clase' : `info` , 'texto' : `Se guardó el documento ${_Codigo} correctamente.` };
        }

        _response.item = await LapTrabajoNoRealizadoModel.findOne({
            where : {
                uu_id : req.body.uu_id
            }
        });
...
router.post('/get_data', async (req,res)=>{
    ...
    try {
        //
        var _Entidad = await LapTrabajoNoRealizadoModel.findOne({
            where : {
                uu_id : req.body.uuid
            }
        })
...
router.put('/:uuid', _Requeridos , async (req,res)=>{
    ...
    try {
        ...
        _response.data = [];
        //
        await LapTrabajoNoRealizadoModel.update(req.body,{
            where : { 
        ...
        var _file = await convertirPDF( 'XLS_TRABAJO_NO_REALIZADO_LAP' , req.body.Codigo );
        // Unir con detalle.
        await LapTrabajoNoRealizadoDetModel.update({
