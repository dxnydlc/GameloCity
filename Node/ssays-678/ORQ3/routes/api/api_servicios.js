...
// Modelos
const { serviciosModel,User } = require('../../db');
...
router.get('/',async(req,res)=>{
	...
	$response.data = await serviciosModel.findAll({
        order : [
            ['Codigo' , 'DESC']
        ],
        limit : 200
...
router.post('/', [
    ...
    //
    await serviciosModel.create(req.body)
    .catch(function (err) {
        $response.estado = 'ERROR';
        $response.error  = err.original.sqlMessage;
        res.json( $response );
    });
    var _dataGuardado = await serviciosModel.findOne({
        where : {
    ...
    if( _dataGuardado ){
        ...
        _Codigo = 'SERV'+_Codigo;
        await serviciosModel.update({
            Codigo : _Codigo
        },{
            where  : {
                uu_id : req.body.uu_id
    ...
    $response.item = await serviciosModel.findOne({
        where : {
            uu_id : req.body.uu_id
        }
    });
...
router.post('/get_data', async (req,res)=>{
    ...
    $response.data = await serviciosModel.findOne({
        where : {
            uu_id : req.body.uuid
        }
    });
...
router.put('/:uuid', [
	...
	await serviciosModel.update(req.body,{
		where : { 
