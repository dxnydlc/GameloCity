...
// api_lap_req_material.js


var _NombreDoc = 'api_lap_req_material';
...
const { xlsLAPReqMatCabModel, xlsLAPReqMatDetModel, User, sequelize, archiGoogleModel, sucursalModel, cargaExcelModelCab } = require('../../db');
...
async function execQuery( _where , _limit  )
{
    ...
    //
    _dataResp = await xlsLAPReqMatCabModel.findAll({
...
router.post('/', _Requeridos ,async (req,res)=>{
    ...
    try {
        ...
        if( _dataGuardado )
        {
            ...
            _Codigo = 'RM'+_Codigo;
            await xlsLAPReqMatCabModel.update({
                Codigo : _Codigo
            },{
                where  : {
                    uu_id : req.body.uu_id
            ...
            await archiGoogleModel.update({
                ...
                where : {
                    token : req.body.uu_id,
                    formulario : 'XLS_REQMATERIAL_LAP'
                }
            });
            var _file = await convertirPDF( 'XLS_REQMATERIAL_LAP' , _Codigo );
            //
            _response.resp = { 'titulo' : `Correcto` , 'clase' : `info` , 'texto' : `Se guardó el documento ${_Codigo} correctamente.` };
        }

        _response.item = await xlsLAPReqMatCabModel.findOne({
            where : {
                uu_id : req.body.uu_id
            }
        });
...
router.post('/get_data', async (req,res)=>{
    ...
    try {
        //
        var _Entidad = await xlsLAPReqMatCabModel.findOne({
            where : {
                uu_id : req.body.uuid
            }
        })
...
router.put('/:uuid', _Requeridos , async (req,res)=>{
    ...
    try {
        ...
        _response.data = [];
        //
        await xlsLAPReqMatCabModel.update(req.body,{
            where : { 
        ...
        var _file = await convertirPDF( 'XLS_REQMATERIAL_LAP' , req.body.Codigo );
        // 342206 343129
            ...
            for(let i = 0; i < sheets.length; i++)
            {
                ...
                await temp.forEach( async (res) => {
                     ...
                     var _dataProk = {
                        ...
                        CodigoProd  : res.Codigo,
                        Descripcion : res.Producto,
                        UnidadMedida : res.UM,
                        Cantidad    : res.Cantidad,
                        Semana01    : res.Semana01,
                        Semana02    : res.Semana02,
                        Semana03    : res.Semana03,
                        Semana04    : res.Semana04
                    };
                    await xlsLAPReqMatDetModel.create( _dataProk )
                    .catch(function (err) {
                        varDump(`${err}.`);
                    });
                })
...
router.post('/items/all', async (req,res)=>{
    ...
    var _NombreCol = `Descripcion`;

    try {
        //
        var _editItem = [ sequelize.fn('CONCAT' , '<button data-nombre="', sequelize.col(_NombreCol) , '" data-id="', sequelize.col('id') , '" data-uuid="', sequelize.col('uu_id') ,'" type="button" class=" itemEdit btn btn-block btn-primary btn-xs"><i class="fa fa-edit" ></i></button>') , 'Editar' ];
        var _DelItem =  [ sequelize.fn('CONCAT' , '<button data-nombre="', sequelize.col(_NombreCol) , '" data-id="', sequelize.col('id') , '" data-uuid="', sequelize.col('uu_id') ,'" type="button" class=" itemDelete btn btn-block btn-danger btn-xs"><i class="fa fa-close" ></i></button>') , 'Anular' ];
...
router.post('/items/del', async (req,res)=>{
    ...
    try {
        //
        await xlsLAPReqMatDetModel.destroy({
            where : {
                uu_id : req.body.uuid
            }
        })
