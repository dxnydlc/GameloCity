...
router.get('/',async(req,res)=>{
	...
	$response.data = await errorLogModel.findAll({
        order : [
            ['id' , 'DESC']
        ],
        limit : 200
...
router.post('/', [
    ...
    //
    await errorLogModel.create(req.body)
    .catch(function (err) {
    ...
    var _dataGuardado = await errorLogModel.findOne({
        where : {
    ...
    $response.item = await errorLogModel.findOne({
        where : {
            uu_id : req.body.uu_id
        }
    });
...
router.post('/get_data', async (req,res)=>{
    ...
    $response.data = await errorLogModel.findOne({
        where : {
            uu_id : req.body.uuid
        }
    })
...
router.put('/:uuid', [
	...
	await errorLogModel.update(req.body,{
		where : { 
