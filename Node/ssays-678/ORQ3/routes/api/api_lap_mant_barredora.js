// api_lap_mant_barredora.js


var _NombreDoc = 'api_lap_mant_barredora';
...
const { xlsLAPMantBarredoraModel, xlsLAPIncidenciasDetModel, User, sequelize, archiGoogleModel, sucursalModel, cargaExcelModelCab } = require('../../db');
...
async function execQuery( _where , _limit  )
{
    ...
    //
    _dataResp = await xlsLAPMantBarredoraModel.findAll({
...
router.post('/', _Requeridos ,async (req,res)=>{
    ...
    try {
        ...
        if( _dataGuardado )
        {
            ...
            _Codigo = 'MB'+_Codigo;
            await xlsLAPMantBarredoraModel.update({
                Codigo : _Codigo
            },{
                where  : {
                    uu_id : req.body.uu_id
            ...
            await archiGoogleModel.update({
                ...
                where : {
                    token : req.body.uu_id,
                    formulario : 'XLS_MANT_BARREDORA_LAP'
                }
            });
            var _file = await convertirPDF( 'XLS_MANT_BARREDORA_LAP' , _Codigo );
            _response.resp = { 'titulo' : `Correcto` , 'clase' : `info` , 'texto' : `Se guardó el documento ${_Codigo} correctamente.` };
        }

        _response.item = await xlsLAPMantBarredoraModel.findOne({
            where : {
                uu_id : req.body.uu_id
            }
        });
...
router.post('/get_data', async (req,res)=>{
    ...
    try {
        //
        var _Entidad = await xlsLAPMantBarredoraModel.findOne({
            where : {
                uu_id : req.body.uuid
            }
        })
...
router.put('/:uuid', _Requeridos , async (req,res)=>{
    ...
    try {
        ...
        _response.data = [];
        //
        await xlsLAPMantBarredoraModel.update(req.body,{
            where : { 
        ...
        // 342206 343129
        var _file = await convertirPDF( 'XLS_MANT_BARREDORA_LAP' , req.body.Codigo );
