...
const { reportesModel,User } = require('../../db');
...
router.get('/',async(req,res)=>{
	...
	$response.data = await reportesModel.findAll({
        order : [
            ['id' , 'DESC']
        ],
        limit : 200
...
router.post('/', [
    ...
    //
    await reportesModel.create(req.body)
    .catch(function (err) {
    ...
    var _dataGuardado = await reportesModel.findOne({
        where : {
    ...
    $response.item = await reportesModel.findOne({
        where : {
            uu_id : req.body.uu_id
        }
    });
...
router.post('/get_data', async (req,res)=>{
    ...
    $response.data = await reportesModel.findOne({
        where : {
            uu_id : req.body.uuid
        }
    })
...
router.put('/:uuid', [
	...
	await reportesModel.update(req.body,{
		where : { 
