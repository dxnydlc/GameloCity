// api_lap_accidentes_incidentes.js


var _NombreDoc = 'api_lap_accidentes_incidentes';
...
const { xlsLAPAccidentesInciModel, xlsLAPIncidenciasDetModel, User, sequelize, archiGoogleModel, sucursalModel, cargaExcelModelCab } = require('../../db');
...
async function execQuery( _where , _limit  )
{
    ...
    //
    _dataResp = await xlsLAPAccidentesInciModel.findAll({
...
router.post('/', _Requeridos ,async (req,res)=>{
    ...
    try {
        ...
        if( _dataGuardado )
        {
            ...
            _Codigo = 'IA'+_Codigo;
            await xlsLAPAccidentesInciModel.update({
                Codigo : _Codigo
            },{
                where  : {
                    uu_id : req.body.uu_id
            ...
            var _file = await convertirPDF( 'XLS_ACCIDENTES_INCIDENTES_LAP' , _Codigo );
            _response.resp = { 'titulo' : `Correcto` , 'clase' : `info` , 'texto' : `Se guardó el documento ${_Codigo} correctamente.` };
        }

        _response.item = await xlsLAPAccidentesInciModel.findOne({
            where : {
                uu_id : req.body.uu_id
            }
        });
...
router.post('/get_data', async (req,res)=>{
    ...
    try {
        //
        var _Entidad = await xlsLAPAccidentesInciModel.findOne({
            where : {
                uu_id : req.body.uuid
            }
        })
...
router.put('/:uuid', _Requeridos , async (req,res)=>{
    ...
    try {
        ...
        _response.data = [];
        //
        await xlsLAPAccidentesInciModel.update(req.body,{
            where : { 
        ...
        var _file = await convertirPDF( 'XLS_ACCIDENTES_INCIDENTES_LAP' , req.body.Codigo );
