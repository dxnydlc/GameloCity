...
var storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, 'adjuntos/constancia_supervision/')
  },
  filename: function (req, file, cb) {
    ...
    // 9999999999999999999999
    cb(null, 'IMG_CONSTSUP_'+Date.now()+'_'+_uuid + '.'+arExt[1] )
  }
});
...
var storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, RUTA_APP+'/adjuntos/constancia_supervision/' )
  },
  filename: function (req, file, cb) {
...
const { User , archivoConstSupModel} = require('../../db');
...
router.post('/carga', uploadCarpeta.single('formData'), async (req, res) => {
    ...
    var $RutaFile = RUTA_APP+'/adjuntos/constancia_supervision/';
    console.log('>>>>>>>>>'+$RutaFile);
    
    //var si = sizeof.SI($RutaFile+$ArchivoFinal);
  
    $dataInsert.formulario      = 'CONSTANCIA_SUPERVISION';
    ...
    await archivoConstSupModel.create( $dataInsert );
    var ItemAgregado = await archivoConstSupModel.findOne({
      where : {
