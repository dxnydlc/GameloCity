...
// Modelos
const { catalogo7Model,User } = require('../../db');
...
router.get('/',async(req,res)=>{
	...
	$response.data = await catalogo7Model.findAll({
        order : [
            ['Codigo' , 'DESC']
        ],
        limit : 200
...
router.post('/', [
    ...
    //
    await catalogo7Model.create(req.body)
    .catch(function (err) {
    ...
    $response.item = await catalogo7Model.findOne({
        where : {
            uu_id : req.body.uu_id
        }
    });
...
router.post('/get_data', async (req,res)=>{
    ...
    $response.data = await catalogo7Model.findOne({
        where : {
            uu_id : req.body.uuid
        }
    });
...
router.put('/:uuid', [
	...
	await catalogo7Model.update(req.body,{
		where : { 
