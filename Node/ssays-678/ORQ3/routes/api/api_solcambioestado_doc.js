// api_solcambioestado_doc.js

var _NombreDoc = 'api_solcambioestado_doc';
...
const { SolCambioEstadoModel,User, SolCambioEstadoModelDet } = require('../../db');

// >>>>>>>>>>>>>    Plantillas Mail    >>>>>>>>>>>>>
const { mail_cambioEstado_Doc } = require( './mail_plantillas' );
...
router.get('/',async(req,res)=>{
	...
	$response.data = await SolCambioEstadoModel.findAll({
        order : [
            ['Codigo' , 'DESC']
        ],
        limit : 200
...
//////////////////////////////////////////////////////////
router.post('/', [
    check('IdAutorizado' ,'Seleccione mes').not().isEmpty(),
    check('Motivo' ,'Ingrese año').not().isEmpty(),
] ,async (req,res)=>{
    ...
    req.body.Fecha  = moment().format('YYYY-MM-DD HH:mm:ss');
    req.body.Mes    = moment().format('MM');
    req.body.Anio   = moment().format('YYYY');
    //
    await SolCambioEstadoModel.create(req.body)
    .catch(function (err) {
    ...
    var _dataGuardado = await SolCambioEstadoModel.findOne({
        where : {
    ...
    if( _dataGuardado ){
        // Numero de items...
        var _NroItems = await SolCambioEstadoModelDet.count({
            where: {
        ...
        _Codigo = 'CE'+_Codigo;
        await SolCambioEstadoModel.update({
            Codigo   : _Codigo ,
            NroItems : _NroItems
        },{
            where  : {
        ...
        // Unir con detalle
        await SolCambioEstadoModelDet.update({
            CodigoCab : _Codigo
        },{
            where : {
                Token : _dataGuardado.uu_id
    ...
    $response.item = await SolCambioEstadoModel.findOne({
        where : {
            uu_id : req.body.uu_id
        }
    });
...
router.post('/get_data', async (req,res)=>{
    ...
    $response.data = await SolCambioEstadoModel.findOne({
        where : {
            uu_id : req.body.uuid
        }
    })
...
router.put('/:uuid', [
	...
	await SolCambioEstadoModel.update(req.body,{
		where : { 
...
router.post('/item/add', async (req,res)=>{
    // NroDoc (,), TDoc, Token, CodCab
    var $response = {};
    $response.estado = 'OK';
    $response.data = [];

    var $ids = req.body.NroDoc;
    var _arIds = $ids.split(',');

    for (var i = 0; i < _arIds.length; i++ ){
        var _OS = _arIds[ i ];
        var _insertar = {};
        _insertar.uu_id     = await renovarToken();
        _insertar.CodigoCab = req.body.CodCab;
        _insertar.TDoc   = req.body.TDoc;
        _insertar.NroDoc = _OS;
        _insertar.Token = req.body.Token;
        await SolCambioEstadoModelDet.create(_insertar)
        .catch(function (err) {
            captueError( err.original , req.body );
            console.log(_NombreDoc);
            $response.estado = 'ERROR';$response.error  = err.original.sqlMessage;res.json( $response );
    ...
    $response.data = await SolCambioEstadoModelDet.findAll({
        where : {
