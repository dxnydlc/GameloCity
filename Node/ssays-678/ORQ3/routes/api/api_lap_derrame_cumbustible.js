// api_lap_derrame_cumbustible.js



var _NombreDoc = 'api_lap_derrame_cumbustible';
...
const { xlsLAPDerrameCombustibleModel, xlsLAPIncidenciasDetModel, User, sequelize, archiGoogleModel, sucursalModel, cargaExcelModelCab } = require('../../db');
...
async function execQuery( _where , _limit  )
{
    ...
    //
    _dataResp = await xlsLAPDerrameCombustibleModel.findAll({
...
router.post('/', _Requeridos ,async (req,res)=>{
    ...
    try {
        ...
        if( _dataGuardado )
        {
            ...
            _Codigo = 'DC'+_Codigo;
            await xlsLAPDerrameCombustibleModel.update({
                Codigo : _Codigo
            },{
                where  : {
                    uu_id : req.body.uu_id
            ...
            await archiGoogleModel.update({
                ...
                where : {
                    token : req.body.uu_id,
                    formulario : 'XLS_ACCIDENTES_INCIDENTES_LAP'
                }
            });
            var _file = await convertirPDF( 'XLS_LAP_DERRAME_COMBUSTIBLE' , _Codigo );
            _response.resp = { 'titulo' : `Correcto` , 'clase' : `info` , 'texto' : `Se guardó el documento ${_Codigo} correctamente.` };
        }

        _response.item = await xlsLAPDerrameCombustibleModel.findOne({
            where : {
                uu_id : req.body.uu_id
            }
        });
...
router.post('/get_data', async (req,res)=>{
    ...
    try {
        //
        var _Entidad = await xlsLAPDerrameCombustibleModel.findOne({
            where : {
                uu_id : req.body.uuid
            }
        })
...
router.put('/:uuid', _Requeridos , async (req,res)=>{
    ...
    try {
        ...
        _response.data = [];
        //
        await xlsLAPDerrameCombustibleModel.update(req.body,{
            where : { 
        ...
        await archiGoogleModel.update({
            ...
            where : {
                token : req.params.uuid ,
                formulario : 'XLS_LAP_DERRAME_COMBUSTIBLE'
            }
        });
        var _file = await convertirPDF( 'XLS_LAP_DERRAME_COMBUSTIBLE' , req.body.Codigo );
