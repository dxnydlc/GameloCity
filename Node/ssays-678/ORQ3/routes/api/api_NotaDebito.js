// api_NotaDebito.js
var _NombreDoc = `api_NotaDebito`, _TipoDocG = `ND`;
...
async function maxeXML( _Data , _Det , $userData )
{
    ...
    // _NombreDoc
    var _Archivo = `ND-${_Data.Serie}-${_Data.Correlativo}.php`;
...
// EL DOCUMENTO ES AFECTO A IGV O NO¿?
var _infoGrabada = ``, _DetalleItem = ``;
if( _Data.OpGravada == 'SI' )
{
    //
    _infoGrabada = `<!-- OPER. GRAVADA CON IMPUESTOS -->
    ...
    //
    _DetalleItem = `<!-- OPER. GRAVADA CON IMPUESTOS -->
    ...
    //
    _infoGrabada = `<!-- OPER. GRAVADA SIN IMPUESTOS -->
    <totalValorVentaNetoOpNoGravada>${parseFloat(_Data.SubTotal_Doc).toFixed(2)}</totalValorVentaNetoOpNoGravada>
    <totalIgv>0.00</totalIgv>
    <totalImpuestos>0.00</totalImpuestos>
    <totalVenta>${parseFloat(_Data.SubTotal_Doc).toFixed(2)}</totalVenta>`;
    //
    _DetalleItem = `<!-- OPER. GRAVADA SIN IMPUESTOS -->
    <montoBaseIgv>0.00</montoBaseIgv>
    <tasaIgv>0.00</tasaIgv>
    <importeTotalImpuestos>0.00</importeTotalImpuestos>
    <importeIgv>0.00</importeIgv>
    <codigoRazonExoneracion>30</codigoRazonExoneracion>
    <importeUnitarioConImpuesto>${parseFloat(_Data.SubTotal_Doc).toFixed(2)}</importeUnitarioConImpuesto>`;
    //
}
...
<parameter value="08" name="tipoDocumento"/>
<documento>
...
<tipoDocumento>08</tipoDocumento>
...
<numeroDocumentoAdquiriente>${_Data.IdCliente}</numeroDocumentoAdquiriente>
...
<codigoSerieNumeroAfectado>01</codigoSerieNumeroAfectado>
...
$myfile = fopen("ND-${_Data.Serie}-${_Data.Correlativo}.xml", "w") or die("Unable to open file!");
...
<html lang="en">
  ...
  <body>
    ...
    <h1>Generando doc: ND-${_Data.Serie}-${_Data.Correlativo}</h1>
    ...
    socket.emit('emitir_doc_ventas',{
        user : $nomU,
        msg  : 'ND-${_Data.Serie}-${_Data.Correlativo}',
        dni  : $dniU
    });
    // ******* NODE JS *******
    </script>
