...
// api_personal_cronograma.js




var _NombreDoc = 'api_personal_cronograma';
...
const { cronogramaPersonalCabModel, cronogramaPersonalDetModel, cronogramaTrabajoDetModel, User, sequelize, archiGoogleModel, sucursalModel } = require('../../db');
...
// VALIDACION
var _Requeridos = [
    check('DNI' ,'Seleccione personal').not().isEmpty() 
];
...
async function headerDetalle( _where , _limit  )
{
    ...
    var _DelItem =  [ sequelize.fn('CONCAT' , '<button data-id="', sequelize.col('id') , '" data-uuid="', sequelize.col('uu_id') ,'" data-nombre="', sequelize.col(_NOmbre) ,'" type="button" class=" delAct btn btn-block btn-danger btn-xs"><i class="fa fa-close" ></i></button>') , 'Anular' ];
    ...
    //
    _dataResp = await cronogramaPersonalDetModel.findAll({
...
async function execQuery( _where , _limit  )
{
    ...
    //
    _dataResp = await cronogramaPersonalCabModel.findAll({
        attributes : _atributos ,
        order : [
            ['Codigo' , 'DESC']
        ],
...
router.post('/', _Requeridos ,async (req,res)=>{
    ...
    try {
        ...
        if( _dataGuardado )
        {
            ...
            _Codigo = 'CU'+_Codigo;
            await cronogramaPersonalCabModel.update({
                Codigo : _Codigo
            },{
                where  : {
                    uu_id : req.body.uu_id
            ...
            // Unir con detalle.
            await cronogramaPersonalDetModel.update({
                CodigoHead : _Codigo
            },{
                where : {
        ...
        _response.item = await cronogramaPersonalCabModel.findOne({
            where : {
                uu_id : req.body.uu_id
            }
        });
...
router.post('/get_data', async (req,res)=>{
    ...
    try {
        //
        var _Entidad = await cronogramaPersonalCabModel.findOne({
            where : {
                uu_id : req.body.uuid
            }
        })
...
router.put('/:uuid', _Requeridos , async (req,res)=>{
    ...
    try {
        ...
        _response.data = [];
        //
        await cronogramaPersonalCabModel.update(req.body,{
            where : { 
...
// -------------------------------------------------------------
//////////////////////////////////////////////////////////
//               AGREGAR ACTIVIADES USUARIO             //
//////////////////////////////////////////////////////////
router.post('/add_actividades', async (req,res)=>{
    // ids(,), Token, CodigoHead
    var _response = {};
    _response.codigo = 200;
    ...
    try {
        //
        var _txtIds = req.body.ids;
        var _arrIds = _txtIds.split(',');
        varDump( _arrIds );
        for (let index = 0; index < _arrIds.length; index++) {
            const _id = _arrIds[index];
            var _item = await cronogramaTrabajoDetModel.findOne({
                where : {
                    id : _id
                }
            });
            if( _item ){
                varDump( _item.Actividad );
                var _Codigo = await helpersController.pad_with_zeroes( _item.id , 8 );
                _Codigo = 'ACT'+_Codigo;
                var _uu_id = await helpersController.renovarToken();
                var _inserta = {
                    uu_id   : _uu_id ,
                    Codigo  : _Codigo ,
                    Hinicio : _item.Hinicio ,
                    HFin    : _item.HFin ,
                    Horario : _item.Horario ,
                ...
                await cronogramaPersonalDetModel.create( _inserta )
                .catch(function (err) {
                    _response.codigo = 500;
                    _response.resp = { 'titulo' : `Error` , 'clase' : `error` , 'texto' :  `Error: ${error}.` };
                });
        ...
        // Actualizar indice.
        await updateIdices( req.body.Token );
        //
        _response.resp = { 'titulo' : `Correcto` , 'clase' : `info` , 'texto' : `Actividades agregadas.` };
        //
    } catch (error) {
        varDump(`Error: ${error}.`);
        _response.codigo = 500;
...
async function updateIdices( Token )
{
    ...
    var _Detalles = await cronogramaPersonalDetModel.findAll({
        where : {
            Token : Token
        },
        order : [
...
//////////////////////////////////////////////////////////
//       DEVOLVEMOS LAS ACTIVIDADES PARA EL PERSONAL     //
//////////////////////////////////////////////////////////
router.post('/get_actividades', async (req,res)=>{
    ...
    try {
        ...
        _response.data = await headerDetalle( _where , 1000  );
        //
        _response.resp = { 'titulo' : `Correcto` , 'clase' : `info` , 'texto' : `Actividades cargadas.` };
        //
    } catch (error) {
        varDump(`Error: ${error}.`);
        _response.codigo = 500;
