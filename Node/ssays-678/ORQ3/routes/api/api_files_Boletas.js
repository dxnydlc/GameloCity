...
var storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, 'adjuntos/boletas/')
  },
  filename: function (req, file, cb) {
    var $ext = file.mimetype;
    var arExt = $ext.split('/');
    cb(null, 'BOLETA_SSAYS_'+Date.now() + '.'+arExt[1] ) //Appending .jpg
  }
});
...
var storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, RUTA_APP+'/adjuntos/boletas/' )
  },
  filename: function (req, file, cb) {
...
router.post('/boleta', uploadCarpeta.single('formData'), async (req, res) => {
    ...
    $dataInsert.formulario      = 'BOLETA_MASIVO';
    ...
    await envioBoletaDetModel.create( _insertDetalle );
