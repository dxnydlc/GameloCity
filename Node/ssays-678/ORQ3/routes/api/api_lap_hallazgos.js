...
// api_lap_hallazgos.js

var _NombreDoc = 'api_lap_hallazgos';
...
const { xlsLAPhallazgoCabModel, User, xlsLAPhallazgoDetModel, archiGoogleModel, sequelize, sucursalModel, cargaExcelModelCab } = require('../../db');
...
async function execQuery( _where , _limit  )
{
    ...
    //
    _dataResp = await xlsLAPhallazgoCabModel.findAll({
...
router.post('/', _Requeridos ,async (req,res)=>{
    ...
    try {
        ...
        if( _dataGuardado )
        {
            ...
            _Codigo = 'HA'+_Codigo;
            await xlsLAPhallazgoCabModel.update({
                Codigo : _Codigo
            },{
                where  : {
                    uu_id : req.body.uu_id
            ...
            await archiGoogleModel.update({
                ...
                where : {
                    token : req.body.uu_id,
                    formulario : 'XLS_HALLAZGOS_LAP'
                }
            });
            var _file = await convertirPDF( 'XLS_HALLAZGOS_LAP' , _Codigo );
            _response.resp = { 'titulo' : `Correcto` , 'clase' : `info` , 'texto' : `Se guardó el documento ${_Codigo} correctamente.` };
        }

        _response.item = await xlsLAPhallazgoCabModel.findOne({
            where : {
                uu_id : req.body.uu_id
            }
        });
...
router.post('/get_data', async (req,res)=>{
    ...
    try {
        //
        var _Entidad = await xlsLAPhallazgoCabModel.findOne({
            where : {
                uu_id : req.body.uuid
            }
        })
...
router.put('/:uuid', _Requeridos , async (req,res)=>{
    ...
    try {
        ...
        _response.data = [];
        //
        await xlsLAPhallazgoCabModel.update(req.body,{
            where : { 
        ...
        var _file = await convertirPDF( 'XLS_HALLAZGOS_LAP' , req.body.Codigo );
