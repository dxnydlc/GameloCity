...
// api_carrito_barredor.js

var _NombreDoc = 'api_carrito_barredor';
...
const { activosModel231 } = require('../../db31');
const { LAPCarritoBarredorCabModel, LAPCarritoBarredorDetModel, User, sequelize, archiGoogleModel, sucursalModel, cargaExcelModelCab } = require('../../db');
...
async function execQuery( _where , _limit  )
{
    ...
    //
    _dataResp = await LAPCarritoBarredorCabModel.findAll({
...
router.post('/', _Requeridos ,async (req,res)=>{
    ...
    try {
        ...
        if( _dataGuardado )
        {
            ...
            _Codigo = 'CB'+_Codigo;
            await LAPCarritoBarredorCabModel.update({
                Codigo : _Codigo
            },{
                where  : {
                    uu_id : req.body.uu_id
            ...
            await archiGoogleModel.update({
                ...
                where : {
                    token : req.body.uu_id,
                    formulario : 'XLS_CARRITO_BARREDOR_LAP'
                }
            });
            var _file = await convertirPDF( 'XLS_CARRITO_BARREDOR_LAP' , _Codigo );
            //
            _response.resp = { 'titulo' : `Correcto` , 'clase' : `info` , 'texto' : `Se guardó el documento ${_Codigo} correctamente.` };
        }

        _response.item = await LAPCarritoBarredorCabModel.findOne({
            where : {
                uu_id : req.body.uu_id
            }
        });
...
router.post('/get_data', async (req,res)=>{
    ...
    try {
        //
        var _Entidad = await LAPCarritoBarredorCabModel.findOne({
            where : {
                uu_id : req.body.uuid
            }
        })
...
router.put('/:uuid', _Requeridos , async (req,res)=>{
    ...
    try {
        ...
        _response.data = [];
        //
        await LAPCarritoBarredorCabModel.update(req.body,{
            where : { 
        ...
        // 342206 343129
        var _file = await convertirPDF( 'XLS_CARRITO_BARREDOR_LAP' , req.body.Codigo );
            ...
            for(let i = 0; i < sheets.length; i++)
            {
                ...
                await temp.forEach( async (res) => {
                    if( res.Fecha )
                    {
                        ...
                        await LAPCarritoBarredorCabModel.create( _dataProk )
                        .catch(function (err) {
                            varDump(`${err}.`);
                        });
                        //
...
router.post('/items/all', async (req,res)=>{
    ...
    try {
        ...
        var _DataSec = await LAPCarritoBarredorDetModel.findAll({
            attributes : _atributos ,
            where : {
                Token : req.body.Token
            },
...
//                  ELMINAR OPERARIO                    //
//////////////////////////////////////////////////////////
router.post('/obs/del', async (req,res)=>{
    var _response = {};
    _response.codigo = 200;
    _response.data = [];
...
//////////////////////////////////////////////////////////
router.post('/obs/guardar', async (req,res)=>{
    var _response = {};
    _response.codigo = 200;
    _response.data = [];
    ...
    try {
        ...
        {
            // Udate
            await LAPCarritoBarredorDetModel.update(req.body,{
                where : {
                    id : _id
                }
            })
            ...
            // Crear
            await LAPCarritoBarredorDetModel.create(req.body)
            .catch(function (err) {
                varDump(err);
                helpersController.captueError( err.original , req.body );
                _response.codigo = 400;
        ...
        //
        var _dataGuardado = await LAPCarritoBarredorDetModel.findOne({
            where : {
                uu_id : req.body.uu_id
            }
        });
        if( _dataGuardado )
        {
            var _Codigo = await helpersController.pad_with_zeroes( _dataGuardado.id , 8 );
            _Codigo = 'OBS'+_Codigo;
            await LAPCarritoBarredorDetModel.update({
                Codigo : _Codigo
            },{
                where  : {
                    uu_id : req.body.uu_id
            ...
            // Unir archivos con observacion con detalle.
            await archiGoogleModel.update({
                Cod001 : _Codigo
            },{
                where : {
        ...
        var _Item = await LAPCarritoBarredorDetModel.findOne({
            where : {
                uu_id : req.body.uu_id
            }
        });
        _response.item = _Item;
...
//                  BUSCAR UN ACTIVO                    //
//////////////////////////////////////////////////////////
router.post('/activo_placa', async (req,res)=>{
    // Placa | 
    var _response = {};
    _response.codigo = 200;
    ...
    try {
        //
        var _Entidad = await activosModel231.findOne({
            where : {
        ...
        if( _Entidad )
        {
            _response.resp = { 'titulo' : `Correcto` , 'clase' : `info` , 'texto' : `Se cargaron los datos del documento: ${_Entidad.Placa}.` };
        }else{
            _response.resp = { 'titulo' : `Error` , 'clase' : `Warning` , 'texto' : `No existe el activo` };
        }
        //
    } catch (error) {
        varDump(error);
...
//////////////////////////////////////////////////////////
//                LISTAR ARCHIVOS ITEM                  //
//////////////////////////////////////////////////////////
router.post('/archivos/all', async (req,res)=>{
    // Token
    var _response = {};
    _response.codigo = 200;
    _response.data = [];
    ...
    try {
        ...
        //
        _response.data = _DataSec;
        _response.resp = { 'titulo' : `Correcto` , 'clase' : `info` , 'texto' : `Archivos cargados correctamente.` };
        //
    } catch (error) {
        //
        varDump(`Error: ${error}.`);
