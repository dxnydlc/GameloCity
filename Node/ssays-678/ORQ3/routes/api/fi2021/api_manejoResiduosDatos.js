// api_manejoResiduosDatos.js


var _NombreDoc = 'api_manejoResiduosDatos';
...
const { manejoResiduosDatosFI21Model,User } = require('../../../db');
...
router.get('/',async(req,res)=>{
	...
	$response.data = await manejoResiduosDatosFI21Model.findAll({
        order : [
            ['Codigo' , 'DESC']
        ],
        limit : 200
...
router.post('/', [
    ...
    //
    await manejoResiduosDatosFI21Model.create(req.body)
    .catch(function (err) {
    ...
    var _dataGuardado = await manejoResiduosDatosFI21Model.findOne({
        where : {
    ...
    if( _dataGuardado ){
        ...
        _Codigo = 'HOR'+_Codigo;
        await manejoResiduosDatosFI21Model.update({
            Codigo : _Codigo
        },{
            where  : {
                uu_id : req.body.uu_id
    ...
    $response.item = await manejoResiduosDatosFI21Model.findOne({
        where : {
            uu_id : req.body.uu_id
        }
    });
...
router.post('/get_data', async (req,res)=>{
    ...
    $response.data = await manejoResiduosDatosFI21Model.findOne({
        where : {
            uu_id : req.body.uuid
        }
    })
...
router.put('/:uuid', [
	...
	await manejoResiduosDatosFI21Model.update(req.body,{
		where : { 
