// api_manejoResiduosFI21.js


var _NombreDoc = 'api_manejoResiduosFI21';
...
const { manejoResiduosFI21Model,User } = require('../../../db');
...
router.get('/',async(req,res)=>{
	...
	$response.data = await manejoResiduosFI21Model.findAll({
        order : [
            ['Codigo' , 'DESC']
        ],
        limit : 200
...
router.post('/', [
    ...
    //
    await manejoResiduosFI21Model.create(req.body)
    .catch(function (err) {
    ...
    var _dataGuardado = await manejoResiduosFI21Model.findOne({
        where : {
    ...
    if( _dataGuardado ){
        ...
        _Codigo = 'HOR'+_Codigo;
        await manejoResiduosFI21Model.update({
            Codigo : _Codigo
        },{
            where  : {
                uu_id : req.body.uu_id
    ...
    $response.item = await manejoResiduosFI21Model.findOne({
        where : {
            uu_id : req.body.uu_id
        }
    });
...
router.post('/get_data', async (req,res)=>{
    ...
    $response.data = await manejoResiduosFI21Model.findOne({
        where : {
            uu_id : req.body.uuid
        }
    })
...
router.put('/:uuid', [
	...
	await manejoResiduosFI21Model.update(req.body,{
		where : { 
