// api_productosF121.js


var _NombreDoc = 'api_productosF121';
...
const { productoFI21Model,User } = require('../../../db');
...
router.get('/',async(req,res)=>{
	...
	$response.data = await productoFI21Model.findAll({
        order : [
            ['Codigo' , 'DESC']
        ],
        limit : 200
...
router.post('/', [
    ...
    check('CostoUnt' ,'Ingrese Costo Untitario').not().isEmpty(),
] ,async (req,res)=>{
    ...
    //
    await productoFI21Model.create(req.body)
    .catch(function (err) {
    ...
    var _dataGuardado = await productoFI21Model.findOne({
        where : {
    ...
    if( _dataGuardado ){
        ...
        _Codigo = 'PRD'+_Codigo;
        await productoFI21Model.update({
            Codigo : _Codigo
        },{
            where  : {
                uu_id : req.body.uu_id
    ...
    $response.item = await productoFI21Model.findOne({
        where : {
            uu_id : req.body.uu_id
        }
    });
...
router.post('/get_data', async (req,res)=>{
    ...
    $response.data = await productoFI21Model.findOne({
        where : {
            uu_id : req.body.uuid
        }
    })
...
router.put('/:uuid', [
	...
	await productoFI21Model.update(req.body,{
		where : { 
...
router.delete('/:uuid', async (req,res)=>{
    ...
    var _Entidad = await productoFI21Model.findOne({
        where : {
            uu_id : req.params.uuid
        }
    });
