...
var _NombreDoc = 'api_localesFI21';
...
const { localesFI21Model, User, fichaInspecCabMode, serviciosLocalFI2021 } = require('../../../db');



// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
localesFI21Model.belongsTo( serviciosLocalFI2021 ,{
	as : 'Serv1', foreignKey 	: 'id',targetKey: 'IdLocal',
});
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
router.get('/:IdFicha',async(req,res)=>{
	...
	$response.data = await localesFI21Model.findAll({
        where : {
...
//////////////////////////////////////////////////////////
router.post('/', [
    check('NombreLocal' ,'Ingrese nombre local.').not().isEmpty(),
    check('IdGiro' ,'Seleccione Giro').not().isEmpty(),
    ...
    //
    await localesFI21Model.create(req.body)
    .catch(function (err) {
    ...
    var _dataGuardado = await localesFI21Model.findOne({
        where : {
    ...
    if( _dataGuardado ){
        ...
        // Ahora creamos el servicio (+) que servirá para agregar.
        var _InsertServicio = {};
        _InsertServicio.uu_id   = await renovarToken();
        _InsertServicio.IdLocal = _dataGuardado.id;
        _InsertServicio.Local   = _dataGuardado.NombreLocal;
        _InsertServicio.IdServicio  = 0;
        _InsertServicio.Servicio    = 'Agregar';
        _InsertServicio.Flag        = '*';
        //
        await serviciosLocalFI2021.create( _InsertServicio );
    }

    $response.item = await localesFI21Model.findOne({
        where : {
            uu_id : req.body.uu_id
        }
    });
...
router.post('/get_data', async (req,res)=>{
    ...
    $response.data = await localesFI21Model.findOne({
        where : {
            uu_id : req.body.uuid
        }
    })
...
router.put('/:uuid', [
	...
	await localesFI21Model.update(req.body,{
		where : { 
