...
const { tanquepozoFI21Model,User } = require('../../../db');
...
router.get('/',async(req,res)=>{
	...
	$response.data = await tanquepozoFI21Model.findAll({
        order : [
            ['Codigo' , 'DESC']
        ],
        limit : 200
...
//////////////////////////////////////////////////////////
router.post('/', [
    check('Tipo' ,'Ingrese Tipo').not().isEmpty(),
    check('Descripcion' ,'Ingrese Descripción').not().isEmpty(),
    ...
    //
    await tanquepozoFI21Model.create(req.body)
    .catch(function (err) {
    ...
    var _dataGuardado = await tanquepozoFI21Model.findOne({
        where : {
            uu_id : req.body.uu_id
        }
    });
    if( _dataGuardado ){
        var _Codigo = await pad_with_zeroes( _dataGuardado.id , 12 );
        _Codigo = 'TZ'+_Codigo;
        await tanquepozoFI21Model.update({
            Codigo : _Codigo
        },{
            where  : {
                uu_id : req.body.uu_id
    ...
    $response.item = await tanquepozoFI21Model.findOne({
        where : {
            uu_id : req.body.uu_id
        }
    });
...
router.post('/get_data', async (req,res)=>{
    ...
    $response.data = await tanquepozoFI21Model.findOne({
        where : {
            uu_id : req.body.uuid
        }
    })
...
router.put('/:uuid', [
	...
	await tanquepozoFI21Model.update(req.body,{
		where : { 
...
router.delete('/:uuid', async (req,res)=>{
	...
	await tanquepozoFI21Model.destroy({
		where : { 
            uu_id : req.params.uuid 
        }
    })
