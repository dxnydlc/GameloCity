// api_files_notificar_usuario.js
...
var storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, 'adjuntos/archivos_notificar_usuario/')
  },
  filename: function (req, file, cb) {
   ...
   // cb(null, 'ARCHIVO_COD_QR_'+Date.now() + '.'+arExt[1] ) //Appending .jpg
    cb(null, 'ARCHIVO_NOT_USUARIO_'+Date.now() + '.'+ext ) // arExt[1] Appending .jpg
  }
});
...
var storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, RUTA_APP+'/adjuntos/archivos_notificar_usuario/' )
  },
  filename: function (req, file, cb) {
...
router.post('/archivo', uploadCarpeta.single('formData'), async (req, res) => {
  ...
  var $RutaFile = RUTA_APP+'adjuntos/archivos_notificar_usuario/';
  var $Url = URL_NODE+'/adjuntos/archivos_notificar_usuario/';
...
//var dimensions = sizeOf($RutaFile+$ArchivoFinal);
  ...
  $dataInsert.formulario      = 'NOTIFICAR_USUARIO';
