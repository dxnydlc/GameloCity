...
// api_lap_kit_antiderrame.js

var _NombreDoc = 'api_lap_kit_antiderrame';
...
const { LAPKitAntiderrameCabModel, xlsLAPIncidenciasDetModel, User, sequelize, archiGoogleModel, sucursalModel, cargaExcelModelCab } = require('../../db');
...
async function execQuery( _where , _limit  )
{
    ...
    //
    _dataResp = await LAPKitAntiderrameCabModel.findAll({
...
router.post('/', _Requeridos ,async (req,res)=>{
    ...
    try {
        ...
        if( _dataGuardado )
        {
            ...
            _Codigo = 'KA'+_Codigo;
            await LAPKitAntiderrameCabModel.update({
                Codigo : _Codigo
            },{
                where  : {
                    uu_id : req.body.uu_id
            ...
            await archiGoogleModel.update({
                ...
                where : {
                    token : req.body.uu_id,
                    formulario : 'XLS_KIT_ANTIDERRAME_LAP'
                }
            });
            var _file = await convertirPDF( 'XLS_KIT_ANTIDERRAME_LAP' , _Codigo );
            _response.resp = { 'titulo' : `Correcto` , 'clase' : `info` , 'texto' : `Se guardó el documento ${_Codigo} correctamente.` };
        }

        _response.item = await LAPKitAntiderrameCabModel.findOne({
            where : {
                uu_id : req.body.uu_id
            }
        });
...
router.post('/get_data', async (req,res)=>{
    ...
    try {
        //
        var _Entidad = await LAPKitAntiderrameCabModel.findOne({
            where : {
                uu_id : req.body.uuid
            }
        })
...
router.put('/:uuid', _Requeridos , async (req,res)=>{
    ...
    try {
        ...
        _response.data = [];
        //
        await LAPKitAntiderrameCabModel.update(req.body,{
            where : { 
        ...
        var _file = await convertirPDF( 'XLS_KIT_ANTIDERRAME_LAP' , req.body.Codigo );
...
router.post('/items/all', async (req,res)=>{
    ...
    try {
        //
        var _editItem = [ sequelize.fn('CONCAT' , '<button data-nombre="', sequelize.col('nombre_archivo') , '" data-uuid="', sequelize.col('uu_id') ,'" type="button" class=" itemEdit btn btn-block btn-primary btn-xs"><i class="fa fa-edit" ></i></button>') , 'Editar' ];
        var _DelItem =  [ sequelize.fn('CONCAT' , '<button data-nombre="', sequelize.col('nombre_archivo') , '" data-id="', sequelize.col('id') , '" data-uuid="', sequelize.col('uu_id') ,'" type="button" class=" itemDelete btn btn-block btn-danger btn-xs"><i class="fa fa-close" ></i></button>') , 'Anular' ];
