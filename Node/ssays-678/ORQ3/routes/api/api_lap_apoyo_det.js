// api_lap_apoyo_det.js



var _NombreDoc = 'api_lap_apoyo_det';
...
const { xlsLAPApoyoDeModel, xlsLAPIncidenciasDetModel, User, sequelize, archiGoogleModel, sucursalModel } = require('../../db');
...
// VALIDACION
var _Requeridos = [
    check('Ubicacion' ,'Ingrese Ubicacion').not().isEmpty()
];
...
async function execQuery( _where , _limit  )
{
    ...
    //
    _dataResp = await xlsLAPApoyoDeModel.findAll({
...
router.post('/lista',async(req,res)=>{
    ...
    try {
        ...
        _response.data = await execQuery( _where , 200  );
        _response.resp = { 'titulo' : `Correcto` , 'clase' : `info` , 'texto' : `Detalle cargado.` };
        //
    } catch (error) {
        helpersController.captueError( '' , `>>>${error}` );
        _response.codigo = 500;
...
router.post('/', _Requeridos ,async (req,res)=>{
    ...
    try {
        ...
        if( _dataGuardado )
        {
            ...
            _Codigo = 'AD'+_Codigo;
            await xlsLAPApoyoDeModel.update({
                Codigo : _Codigo
            },{
                where  : {
                    uu_id : req.body.uu_id
            ...
            // var _file = await convertirPDF( 'LAP_APOYO' , _Codigo );
            _response.resp = { 'titulo' : `Correcto` , 'clase' : `info` , 'texto' : `Se guardó el documento ${_Codigo} correctamente.` };
        }

        _response.item = await xlsLAPApoyoDeModel.findOne({
            where : {
                uu_id : req.body.uu_id
            }
        });
...
router.post('/get_data', async (req,res)=>{
    ...
    try {
        //
        var _Entidad = await xlsLAPApoyoDeModel.findOne({
            where : {
                uu_id : req.body.uuid
            }
        })
...
router.put('/:uuid', _Requeridos , async (req,res)=>{
    ...
    try {
        ...
        _response.data = [];
        //
        await xlsLAPApoyoDeModel.update(req.body,{
            where : { 
        ...
        //var _file = await convertirPDF( 'LAP_APOYO' , req.body.Codigo );
...
router.delete('/:uuid', async (req,res)=>{
    ...
    try {
        ...
        var $userData = await helpersController.getUserData( req.headers['api-token'] );
        //
        await xlsLAPApoyoDeModel.destroy({
            where : { 
                uu_id : req.params.uuid 
            }
        })
        ...
        _response.resp = { 'titulo' : `Correcto` , 'clase' : `info` , 'texto' : `Item eliminado.` };
        //
    } catch (error) {
        varDump(`Error: ${error}.`);
        _response.codigo = 500;
