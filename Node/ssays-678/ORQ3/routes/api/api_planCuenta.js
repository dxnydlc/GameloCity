...
// Modelos
const { planCuentaModel,User } = require('../../db');
...
router.get('/',async(req,res)=>{
	...
	$response.data = await planCuentaModel.findAll({
        order : [
            ['Codigo' , 'DESC']
        ],
        limit : 200
...
router.post('/', [
    ...
    //
    await planCuentaModel.create(req.body)
    .catch(function (err) {
    ...
    $response.item = await planCuentaModel.findOne({
        where : {
            uu_id : req.body.uu_id
        }
    });
...
router.post('/get_data', async (req,res)=>{
    ...
    $response.data = await planCuentaModel.findOne({
        where : {
            uu_id : req.body.uuid
        }
    });
...
router.put('/:uuid', [
	...
	await planCuentaModel.update(req.body,{
		where : { 
