...
// api_productosOT22.js





var _NombreDoc = 'api_productosOT22';
...
// Modelos
const { productosOTModel, productosModel } = require('../../db31');
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
productosOTModel.belongsTo( productosModel ,{
...
async function execQuery( _where , _limit  )
{
    ...
    //
    _dataResp = await productosOTModel.findAll({
...
//      			OBTENER ULTIMOS 200     			//
//////////////////////////////////////////////////////////
router.get('/:IdOT',async(req,res)=>{
    var _response = {};
    _response.codigo = 200;
    _response.data = [];

    try {
        varDump( `Cargar productos de OT >>>>${req.params.IdOT}.` );
        _response.ut   = await helpersController.getUserData( req.headers['api-token'] );
        //
        _response.data = await productosOTModel.findAll({
			order: [
				['IdProducto', 'DESC']
			],
		    where: {
		        IdOT : req.params.IdOT , 
...
router.post('/', _Requeridos ,async (req,res)=>{
    ...
    try {
        ...
        _response.item = await productosOTModel.findOne({
            where : {
                uu_id : req.body.uu_id
            }
        });
...
router.post('/get_data', async (req,res)=>{
    ...
    try {
        //
        var _Entidad = await productosOTModel.findOne({
            where : {
                uu_id : req.body.uuid
            }
        })
