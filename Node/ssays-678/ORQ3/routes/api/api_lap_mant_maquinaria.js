// api_lap_mant_maquinaria.js

var _NombreDoc = 'api_lap_mant_maquinaria';
...
const { xlsLAPMantMaqCabModel, xlsLAPMantMaqDetModel, User, sequelize, archiGoogleModel, sucursalModel, xlsLAPMantMaqPartesModel, cargaExcelModelCab } = require('../../db');
...
async function execQuery( _where , _limit  )
{
    //
    var _dataResp = [];
    var _editItem = [ sequelize.fn('CONCAT' , '<button data-id="', sequelize.col('id') , '" data-uuid="', sequelize.col('uu_id') ,'" data-nombre="', sequelize.col('Codigo') ,'" type="button" class=" docEdit btn btn-block btn-primary btn-xs"><i class="fa fa-edit" ></i></button>') , 'Editar' ];
    var _DelItem =  [ sequelize.fn('CONCAT' , '<button data-id="', sequelize.col('id') , '" data-uuid="', sequelize.col('uu_id') ,'" data-nombre="', sequelize.col('Codigo') ,'" type="button" class=" docDelete btn btn-block btn-danger btn-xs"><i class="fa fa-close" ></i></button>') , 'Anular' ];
    ...
    //
    _dataResp = await xlsLAPMantMaqCabModel.findAll({
...
router.post('/', _Requeridos ,async (req,res)=>{
    ...
    try {
        ...
        if( _dataGuardado )
        {
            ...
            _Codigo = 'MM'+_Codigo;
            await xlsLAPMantMaqCabModel.update({
                Codigo : _Codigo
            },{
                where  : {
                    uu_id : req.body.uu_id
            ...
            await archiGoogleModel.update({
                ...
                where : {
                    token : req.body.uu_id,
                    formulario : 'XLS_MANT_MAQUINARIA_LAP'
                }
            });
            var _file = await convertirPDF( 'XLS_MANT_MAQUINARIA_LAP' , _Codigo );
            _response.resp = { 'titulo' : `Correcto` , 'clase' : `info` , 'texto' : `Se guardó el documento ${_Codigo} correctamente.` };
        }

        _response.item = await xlsLAPMantMaqCabModel.findOne({
            where : {
                uu_id : req.body.uu_id
            }
        });
...
router.post('/get_data', async (req,res)=>{
    ...
    try {
        //
        var _Entidad = await xlsLAPMantMaqCabModel.findOne({
            where : {
                uu_id : req.body.uuid
            }
        })
...
router.put('/:uuid', _Requeridos , async (req,res)=>{
    ...
    try {
        ...
        _response.data = [];
        //
        await xlsLAPMantMaqCabModel.update(req.body,{
            where : { 
        ...
        var _file = await convertirPDF( 'XLS_MANT_MAQUINARIA_LAP' , req.body.Codigo );
        ...
        if( _Archivo )
        {
            ...
            for(let i = 0; i < sheets.length; i++)
            {
                ...
                await temp.forEach( async (res) => {
                    ...
                    await xlsLAPMantMaqDetModel.create( _dataProk )
                    .catch(function (err) {
                        varDump(`${err}.`);
                    });
                    //
...
router.post('/items/all', async (req,res)=>{
    ...
    var _editItem = [ sequelize.fn('CONCAT' , '<button data-id="', sequelize.col('id') , '" data-uuid="', sequelize.col('uu_id') ,'" data-nombre="', sequelize.col('Placa') ,'" type="button" class=" itemEdit btn btn-block btn-primary btn-xs"><i class="fa fa-edit" ></i></button>') , 'Editar' ];
    var _DelItem =  [ sequelize.fn('CONCAT' , '<button data-id="', sequelize.col('id') , '" data-uuid="', sequelize.col('uu_id') ,'" data-nombre="', sequelize.col('Placa') ,'" type="button" class=" itemDelete btn btn-block btn-danger btn-xs"><i class="fa fa-close" ></i></button>') , 'Anular' ];

    var _Partes = [ sequelize.fn('CONCAT' , '<button data-id="', sequelize.col('id') , '" data-uuid="', sequelize.col('uu_id') ,'" data-nombre="', sequelize.col('Placa') ,'" type="button" class=" itemPartes btn btn-block btn-primary btn-xs"><i class="fa fa-calendar" ></i></button>') , 'Partes' ];
    ...
    $response.data = await xlsLAPMantMaqDetModel.findAll({
        attributes : _atributos ,
        where : {
            Token : req.body.Token
        },
...
router.post('/items/del', async (req,res)=>{
    ...
    await xlsLAPMantMaqDetModel.destroy({
        where : {
            uu_id : req.body.uuid
        }
    })
...
router.post('/get_files', async (req,res)=>{
    ...
    $response.data = [];
    //
    var _ArchivoG = await archiGoogleModel.findAll({
        where : {
            token : req.body.Token ,
            extension : 'pdf'
        }
    });
    $response.data = _ArchivoG;
    //
    res.json( $response );
});
// -------------------------------------------------------------
...
//////////////////////////////////////////////////////////
//                  DEL PARTES MAQUINARIA               //
//////////////////////////////////////////////////////////
router.post('/partes/del', async (req,res)=>{
    var _response = {};
    _response.codigo = 200;
    _response.data = [];
    ...
    try {
        //
        await xlsLAPMantMaqPartesModel.destroy({
            where : {
                uu_id : req.body.uuid
            }
        })
...
//////////////////////////////////////////////////////////
//                  ADD PARTES MAQUINARIA               //
//////////////////////////////////////////////////////////
router.post('/partes/add', async (req,res)=>{
    var _response = {};
    _response.codigo = 200;
    _response.data = [];
    ...
    try {
        //
        await xlsLAPMantMaqPartesModel.create(req.body)
        .catch(function (err) {
            captueError( err.original , req.body );
            _response.codigo = 400;
            _response.resp = { 'titulo' : `Error` , 'clase' : `error` , 'texto' :  err.original.sqlMessage };
        });
        _response.resp = { 'titulo' : `Correcto` , 'clase' : `info` , 'texto' : `Se agregó el registro correctamente.` };
        //
    } catch (error) {
        //
        varDump(`Error: ${error}.`);
...
//////////////////////////////////////////////////////////
//               LISTAR PARTES MAQUINARIA               //
//////////////////////////////////////////////////////////
router.post('/partes/all', async (req,res)=>{
    // Token
    var $response = {};
    $response.estado = 'OK';
    $response.data = [];
    ...
    var _editItem = [ sequelize.fn('CONCAT' , '<button data-id="', sequelize.col('id') , '" data-uuid="', sequelize.col('uu_id') ,'" data-nombre="', sequelize.col('Parte') ,'" type="button" class=" itemEdit btn btn-block btn-primary btn-xs"><i class="fa fa-edit" ></i></button>') , 'Editar' ];
    var _DelItem =  [ sequelize.fn('CONCAT' , '<button data-id="', sequelize.col('id') , '" data-uuid="', sequelize.col('uu_id') ,'" data-nombre="', sequelize.col('Parte') ,'" type="button" class=" itemDelete btn btn-block btn-danger btn-xs"><i class="fa fa-close" ></i></button>') , 'Anular' ];
    ...
    $response.data = await xlsLAPMantMaqPartesModel.findAll({
        attributes : _atributos ,
        where : {
            Token : req.body.Token
        },
