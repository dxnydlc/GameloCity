...
const { feedbackLAPModel, xlsLAPIncidenciasDetModel, User, sequelize, archiGoogleModel, sucursalModel } = require('../../db');
...
// VALIDACION
var _Requeridos = [
    check('Glosa' ,'Ingrese comentarios').not().isEmpty()
];
...
async function execQuery( _where , _limit  )
{
    ...
    //
    _dataResp = await feedbackLAPModel.findAll({
...
router.post('/', _Requeridos ,async (req,res)=>{
    ...
    try {
        ...
        //
        await feedbackLAPModel.create(req.body)
        .catch(function (err) {
            varDump( err );
            _response.codigo = 500;
            _response.resp = { 'titulo' : `Error` , 'clase' : `error` , 'texto' :  `Error: ${err.original}.` };
        });
        var _dataGuardado = await feedbackLAPModel.findOne({
            where : {
        ...
        if( _dataGuardado )
        {
            ...
            await clientMail.transmissions.send({
                ...
                content: {
                    ...
                    subject : 'FeebBack informe de gestión - LAP #'+_Codigo,
                    html    : req.body.Glosa + '<br/>Atte.'+$userData.name
                },
                recipients : 
                    [
                        { address : 'ddelacruz@ssays-orquesta.com' } ,
                        { address : 'operaciones@ssays.com' } ,
                    ]
            })
            .then( async data => {
                //
        ...
        _response.item = await feedbackLAPModel.findOne({
            where : {
                uu_id : req.body.uu_id
            }
        });
...
router.post('/get_data', async (req,res)=>{
    ...
    try {
        //
        var _Entidad = await feedbackLAPModel.findOne({
            where : {
                CodigoInforme : req.body.Codigo
            }
        })
        .catch(function (err) {
            varDump( err );
...
router.put('/:uuid', _Requeridos , async (req,res)=>{
    ...
    try {
        ...
        _response.data = [];
        //
        await feedbackLAPModel.update(req.body,{
            where : { 
...
async function getUserData( $token )
{
    ...
    var _antributos = [
        ...
        ['cliente','IdCliente'], 'cliente' , ['nombre_cliente','Cliente'], ['sucursal','IdLocal'], ['nombre_local','Local']
    ];
