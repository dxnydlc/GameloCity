...
router.get('/',async(req,res)=>{
	...
	$response.data = await utilidadesModel.findAll({
        order : [
            ['IdMovilidad' , 'DESC']
        ],
        limit : 200
...
router.post('/', [
    ...
    await utilidadesModel.create(req.body)
    .catch(function (err) {
    ...
    var _dataGuardado = await utilidadesModel.findOne({
        where : {
    ...
    $response.item = await utilidadesModel.findOne({
        where : {
            uu_id : req.body.uu_id
        }
    });
...
router.post('/get_data', async (req,res)=>{
    ...
    $response.data = await utilidadesModel.findOne({
        where : {
            uu_id : req.body.uuid
        }
    });
...
router.put('/:uuid', [
	...
	await utilidadesModel.update(req.body,{
		where : { 
            uu_id : req.body.uu_id 
        }
    })
