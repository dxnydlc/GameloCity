...
// Modelos
const { turnoCabModel, User, sucursalModel } = require('../../db');
...
router.get('/',async(req,res)=>{
	...
	$response.data = await turnoCabModel.findAll({
        order : [
            ['Codigo' , 'DESC']
        ],
        limit : 200
...
router.post('/', [
    ...
    //
    await turnoCabModel.create(req.body)
    .catch(function (err) {
        $response.estado = 'ERROR';
        $response.error  = err.original.sqlMessage;
        res.json( $response );
    });
    var _dataGuardado = await turnoCabModel.findOne({
        where : {
    ...
    $response.item = await turnoCabModel.findOne({
        where : {
            uu_id : req.body.uu_id
        }
    });
...
router.post('/get_data', async (req,res)=>{
    ...
    var _Entidad = await turnoCabModel.findOne({
        where : {
            uu_id : req.body.uuid
        }
    });
...
router.put('/:uuid', [
    ...
    // Auditoria
    delete req.body.id;
	await turnoCabModel.update(req.body,{
		where : { 
