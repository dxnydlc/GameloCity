...
var _NombreDoc = 'api_lap_peso_fod';
...
const { LapPesoFodModel, xlsLAPIncidenciasDetModel, User, sequelize, archiGoogleModel, sucursalModel } = require('../../db');
...
async function execQuery( _where , _limit  )
{
    ...
    //
    _dataResp = await LapPesoFodModel.findAll({
    ...
    try {
        ...
        if( _dataGuardado )
        {
            ...
            await archiGoogleModel.update({
                ...
                where : {
                    token : req.body.uu_id ,
                    formulario : 'XLS_PESO_FOD_LAP'
                }
            });
            var _file = await convertirPDF( 'XLS_PESO_FOD_LAP' , _Codigo );
            _response.resp = { 'titulo' : `Correcto` , 'clase' : `info` , 'texto' : `Se guardó el documento ${_Codigo} correctamente.` };
        }

        _response.item = await LapPesoFodModel.findOne({
            where : {
                uu_id : req.body.uu_id
            }
        });
...
router.post('/get_data', async (req,res)=>{
    ...
    try {
        //
        var _Entidad = await LapPesoFodModel.findOne({
            where : {
                uu_id : req.body.uuid
            }
        })
...
router.put('/:uuid', _Requeridos , async (req,res)=>{
    ...
    try {
        ...
        _response.data = [];
        //
        await LapPesoFodModel.update(req.body,{
            where : { 
        ...
        var _file = await convertirPDF( 'XLS_PESO_FOD_LAP' , req.body.Codigo );
        // 342206 343129
