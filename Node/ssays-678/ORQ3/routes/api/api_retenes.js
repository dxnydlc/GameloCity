...
const { retenesModel,User } = require('../../db');
...
router.get('/',async(req,res)=>{
	...
	$response.data = await retenesModel.findAll({
        order : [
            ['id' , 'DESC']
        ]
    });
...
//////////////////////////////////////////////////////////
router.post('/', [
    check('DNI' ,'Seleccione un reten').not().isEmpty(),
    check('IdCliente' ,'Seleccione un cliente').not().isEmpty(),
    check('IdLocal' ,'Seleccione un local').not().isEmpty(),
    check('Fecha' ,'Seleccione una fecha').not().isEmpty(),
    check('IdHorario' ,'Seleccione un horario').not().isEmpty(),
] ,async (req,res)=>{
    ...
    //
    $response.data = await retenesModel.create(req.body)
    .catch(function (err) {
        // handle error;
    ...
    $response.item = await retenesModel.findOne({
        where : {
...
router.post('/get_data', async (req,res)=>{
    ...
    $response.data = await retenesModel.findOne({
        where : {
            uu_id : req.body.uuid
        }
    });
...
router.put('/:uuid', [
	...
	await retenesModel.update(req.body,{
		where : { 
...
router.delete('/:uuid', async (req,res)=>{
	...
	await retenesModel.update({
        Estado              : 'Anulado',
        IdUsuarioAnulado    : $userData.dni,
        AnuladoPor          : $userData.name,
    },{
		where : { 
            uu_id : req.params.uuid 
