...
var _NombreDoc = 'api_lap_personal_aid';
...
const { LapPersonalAIDModel, xlsLAPIncidenciasDetModel, User, sequelize, archiGoogleModel, sucursalModel } = require('../../db');
...
async function execQuery( _where , _limit  )
{
    ...
    //
    _dataResp = await LapPersonalAIDModel.findAll({
...
router.post('/', _Requeridos ,async (req,res)=>{
    ...
    try {
        ...
        if( _dataGuardado )
        {
            ...
            _Codigo = 'AID'+_Codigo;
            await LapPersonalAIDModel.update({
                Codigo : _Codigo
            },{
                where  : {
                    uu_id : req.body.uu_id
            ...
            await archiGoogleModel.update({
                ...
                where : {
                    token : req.body.uu_id ,
                    formulario : 'XLS_PERSONAL_AID_LAP'
                }
            });
            var _file = await convertirPDF( 'XLS_PERSONAL_AID_LAP' , _Codigo );
            _response.resp = { 'titulo' : `Correcto` , 'clase' : `info` , 'texto' : `Se guardó el documento ${_Codigo} correctamente.` };
        }

        _response.item = await LapPersonalAIDModel.findOne({
            where : {
                uu_id : req.body.uu_id
            }
        });
...
router.post('/get_data', async (req,res)=>{
    ...
    try {
        //
        var _Entidad = await LapPersonalAIDModel.findOne({
            where : {
                uu_id : req.body.uuid
            }
        })
...
router.put('/:uuid', _Requeridos , async (req,res)=>{
    ...
    try {
        ...
        _response.data = [];
        //
        await LapPersonalAIDModel.update(req.body,{
            where : { 
        ...
        var _file = await convertirPDF( 'XLS_PERSONAL_AID_LAP' , req.body.Codigo );
        // 342206 343129
