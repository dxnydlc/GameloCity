...
function ejecutarDoc( tipoReq ) {
    try {
        ...
        .done( handleSuccess )
        .fail( handleFail )
        .always( () => ocultarLoader( '#wrapper_form' ) );

    } catch (error) {
        alert(error);
        ocultarLoader('#wrapper_form');
    }
}
/* ------------------------------------------------------------- */
/* ------------------------------------------------------------- */
function prepararRequest( tipoReq ) {
    let data          = $('#frmDocumento').serialize();
    let id            = parseInt( $('#frmDocumento #id').val() );
    ...
    return {
        url     : esEdicion ? `${urlServicio}actualizar/${uu_id}` : `${urlServicio}guardar`,
        metodo  : esEdicion ? 'PATCH' : 'POST',
        data 
    };
}
...
/* ------------------------------------------------------------- */
/* ------------------------------------------------------------- */
function handleSuccess(json, textStatus, xhr) {
